...
<x-app-layout>
    <div class="my-2">
        <img src="{{ asset($comunidad->banner) }}" alt="{{$comunidad->nombre}} banner" class="w-full h-15px dark:text-white">
    </div>
    
    <div class="dark:text-white flex justify-between items-center px-5 py-3">
        ...
        <div class="flex items-center space-x-4">
            ...
            <form method="POST" action="{{ route('comunidades.destroy', $comunidad->id)}}">
                ...
                <button type="submit" class="flex items-center space-x-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 dark:fill-white" viewBox="0 0 448 512">
            ...
            <a class="flex items-center space-x-2" href="{{ route('comunidades.edit', $comunidad->id) }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 dark:fill-white" viewBox="0 0 512 512">
            ...
            <form action="{{ route('comunidad.follower', $comunidad->id) }}" method="post">
                @csrf
                <button class="inline-flex items-center mr-2 px-4 py-2 bg-blue-800 dark:bg-blue-500 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                    Unirse
                </button>
            </form>
        </div>
