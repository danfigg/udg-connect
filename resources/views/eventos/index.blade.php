<x-app-layout>
    <main class="flex flex-col items-center gap-6">
        ...
        @foreach ($eventos as $evento)
            <div class="evento text-black py-5 px-7 border-gray-500 border rounded min-w-[300px] max-w-xl">
                <h1 class="text-xl">{{ $evento->nombre }}</h2>
                <p class="text-gray-600 text-sm">Descripción: {{ $evento->descripcion }}</p>
                <p class="text-gray-600 text-sm">Fecha y Hora: {{ $evento->fecha_hora_evento }}</p>
                <p class="text-gray-600">Banner: {{ $evento->banner }}</p>
